...
::TaintLogRecord::TaintType TaintTypeToRecordEnum(TaintType type) {
  switch (type & TaintType::TAINT_TYPE_MASK) {
    ...
    // Add by Inactive
    case TaintType::INACTIVE:
      return TaintLogRecord::TaintType::INACTIVE;
...
void FlattenTaintData(T* source, TaintData* dest,
  ...
  //   std::cout << " dest: " << IsValidTaintType(static_cast<TaintType>(*dest)) << std::endl;
  // }
}
...
::TaintLogRecord::SinkType FromSinkType(TaintSinkLabel label) {
  switch (label) {
    ...
    // Add by client-pp
    case TaintSinkLabel::PROTOTYPE_POLLUTION:
      return ::TaintLogRecord::SinkType::PROTOTYPE_POLLUTION;
    case TaintSinkLabel::LOGICAL:
      return ::TaintLogRecord::SinkType::LOGICAL;
    case TaintSinkLabel::SET_TAINT_ATTRIBUTE:
      return ::TaintLogRecord::SinkType::SET_TAINT_ATTRIBUTE;
    case TaintSinkLabel::XMLHTTPREQUEST:
      return ::TaintLogRecord::SinkType::XMLHTTPREQUEST;
    default:
      UNREACHABLE();
  }
}
...
int64_t LogIfTainted(IsTaintedVisitor& visitor,
  ...
  StringStream stream(
      &alloc, StringStream::ObjectPrintMode::kPrintObjectConcise);
  isolate->PrintStack(&stream, Isolate::PrintStackMode::kPrintStackConcise);
...
int64_t LogStackTrace(IsTaintedVisitor& visitor,
                     const JsStringInitializer& initer,
                     v8::internal::Isolate* isolate,
                     v8::String::TaintSinkLabel label,
                     std::shared_ptr<SymbolicState> symbolic_data) {
...
int64_t LogForStackTrace(Handle<String> str,
                     TaintSinkLabel label,
                     int symbolic_data) {
  IsTaintedVisitor visitor;
  {
