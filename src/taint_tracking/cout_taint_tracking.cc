...
void FlattenTaintData(T* source, TaintData* dest,
  ...
  uint8_t dest_type = static_cast<uint8_t>(*dest) & TaintType::TAINT_TYPE_MASK;
  if (dest_type != TaintType::UNTAINTED) {
    std::cout << "FlattenTaintData source: ";
    source->Print();
    std::cout << " dest: " << TaintTypeToString(static_cast<TaintType>(dest_type)) << std::endl;
  }
}
...
void CopyIn(T* dest, TaintType source, int offset, int len) {
  ...
  // std::cout << "CopyIn1 dest: ";
  // dest->Print();
  // std::cout << " source: " << TaintTypeToString(source) << std::endl;
}
...
void OnNewDeserializedString(String* source) {
  ...
  int64_t taint = source->taint_info();
  if (taint != TaintType::UNTAINTED) {
    std::cout << "NewDeserializedString " << taint <<"|";
    source->Print();
    std::cout << std::endl;
  }
...
void OnNewSubStringCopy(T* source, S* dest, int offset, int length) {
  ...
  int64_t staint = source->taint_info();
  int64_t dtaint = dest->taint_info();
  if (staint != TaintType::UNTAINTED || dtaint != TaintType::UNTAINTED) {
    std::cout << "OnNewSubStringCopy from " << staint <<"|"; 
    source->Print();
    std::cout << " to " << dtaint <<"|";
    dest->Print();
...
void OnNewSlicedString(SlicedString* target, String* first,
  ...
  int64_t staint = target->taint_info();
  int64_t dtaint = first->taint_info();
  if (staint != TaintType::UNTAINTED || dtaint != TaintType::UNTAINTED) {
    std::cout << "NewSlicedString target " << staint <<"|";
    target->Print();
    std::cout << " first " << dtaint <<"|";
    first->Print();
    std::cout << " " << offset << " " << length << std::endl;
  }
...
void OnNewConcatStringCopy(T* dest, S* first, R* second) {
  ...
  int64_t ftaint = first->taint_info();
  int64_t dtaint = second->taint_info();
  if (dtaint != TaintType::UNTAINTED || ftaint != TaintType::UNTAINTED) {
    std::cout<< "ConcatTaint dest " << *staint << "|";
    dest->Print();
    std::cout << " first " << ftaint << "|"; 
    first->Print();
    std::cout << " second " << dtaint << "|";
    second->Print();
    std::cout << std::endl;
  }
...
void OnNewConsString(ConsString* target, String* first, String* second) {
  ...
  int64_t ftaint = first->taint_info();
  int64_t dtaint = second->taint_info();
  if (staint != TaintType::UNTAINTED || dtaint != TaintType::UNTAINTED || ftaint != TaintType::UNTAINTED) {
    std::cout << "NewConsString target " << staint << "|";
    target->Print();
    std::cout << " first " << ftaint << "|";
    first->Print();
    std::cout << " second " << dtaint << "|";
...
template <class T> void OnNewExternalString(T* str) {
  ...
  int64_t taint = str->taint_info();
  if (taint != TaintType::UNTAINTED) {
    std::cout << "NewExternalString: " << taint << "|";
    str->Print();
    std::cout << std::endl;
  }
}
...
void OnConvertCase(String* source, T* answer) {
  ...
  int64_t staint = source->taint_info();
  int64_t dtaint = answer->taint_info();
  if (staint != TaintType::UNTAINTED || dtaint != TaintType::UNTAINTED) {
    std::cout << "ConvertCase from " << staint << "|";
    source->Print();
    std::cout << " to " << dtaint << "|";
    answer->Print();
    std::cout << std::endl;
