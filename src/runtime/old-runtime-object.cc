...
MaybeHandle<Object> Runtime::GetObjectProperty(
  ...
  // Added by zfk
  if (!result.is_null() && it.IsFound() && key->IsString()) {
    ...
    if (is_key_tainted) {
      // Taint result
      // result->MarkAsPrototypePollutionTainted();
      // Object *obj_result = result->value();
      // obj_result->MarkAsPrototypePollutionTainted();
      Handle<Object> handle_result = result.ToHandleChecked();
      // handle_result->MarkAsPrototypePollutionTainted();
      // Object *obj_result = *handle_result;
      // if (handle_result->IsJSObject()) {
      //   JSObject *obj_heap_result = JSObject::cast(*handle_result);
      //   obj_heap_result->MarkAsPrototypePollutionTainted();
      // }
      if (handle_result->IsHeapObject()) {
        handle_result->SetIsPrototypePolluted();
        std::cout << "ObjTaintedDueToTaintKey! ObjectAddress: " << handle_result->address() << " KeyTaintType " << tainttracking::TaintTypeToString(key_taint_type) << " KeyIs ";
        key->Print();
        std::cout << std::endl;
      }
  ...
  // if (object->IsJSObject()) {
    ...
    if (key->IsString() && value->IsString()) {
      if (object->IsHeapObject() && object->IsPrototypePolluted()) {
        ...
        tainttracking::TaintType value_taint_type = tainttracking::GetTaintStatusRange(string_value, 0, string_value->length());
        bool is_value_tainted = value_taint_type != tainttracking::TaintType::UNTAINTED;
        if (is_key_tainted && is_value_tainted) {
          // object, key, value should all be tainted
          // LogPrototypePollutionToFile();
          int64_t ret = tainttracking::LogIfTainted(Handle<String>::cast(value), v8::String::TaintSinkLabel::PROTOTYPE_POLLUTION, 0);

          std::cout << "ppFOUND! ObjectAddress " << object->address() << " KeyTaintType " << tainttracking::TaintTypeToString(key_taint_type) << " ValueTaintType " << tainttracking::TaintTypeToString(value_taint_type) << " MessageId " << ret << " KeyIs ";
          // object->Print();
          key->Print();
	  std::cout << " ValueIs ";
          value->Print();
